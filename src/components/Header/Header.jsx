                ...
                <Container>
                    ...
                    <Navbar.Collapse id="basic-navbar-nav">
                        <Nav className="ms-auto">
                            ...
                            {user.email ? (<Nav.Link as={NavLink} to=""> {user?.photoURL ? (<Image src={user?.photoURL} style={{ width: '30px' }} roundedCircle />) : (<div className="bg-primary p-1" style={{ borderRadius: '50px' }}>{user?.displayName?.slice(0, 2)}</div>)} </Nav.Link>) : <Nav.Link as={NavLink} to="/register">Register</Nav.Link>}
